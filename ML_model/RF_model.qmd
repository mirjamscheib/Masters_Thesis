...
write.csv(pdp_model_y_v_k, "ML_Model/pdp_data/pdp_y_v_k_c_rf.csv", row.names = FALSE)
...
write.csv(pdp_model_y_v_s, "ML_Model/pdp_data/pdp_y_v_s_c_rf.csv", row.names = FALSE)
...
```{r}
pdp_model_v_ra <- pdp_model_v_ra |>
  mutate(model = "velocity w/ coordinates")
pdp_model_v_ra_c <- pdp_model_v_ra_c |>
  mutate(model = "velocity w/o coordinates")

pdp_model_v_wd_ra <- pdp_model_v_wd_ra |>
  mutate(model = "v + wd w/ coordinates")
pdp_model_v_wd_ra_c <- pdp_model_v_wd_ra_c |>
  mutate(model = "v + wd w/o coordinates")

pdp_model_v_samp_ra <- pdp_model_v_samp_ra |>
  mutate(model = "samp_locations w/ coordinates")
pdp_model_v_samp_ra_c <- pdp_model_v_samp_ra_c |>
  mutate(model = "samp_locations w/o coordinates")

pdp_model_v_sec_ra <- pdp_model_v_sec_ra |>
  mutate(model = "all predictors w/ coordinates")
pdp_model_v_sec_ra_c <- pdp_model_v_sec_ra_c |>
  mutate(model = "all predictors w/o coordinates")

test_v <- rbind(pdp_model_v_sec_ra_c, pdp_model_v_sec_ra, pdp_model_v_samp_ra_c, pdp_model_v_samp_ra, pdp_model_v_wd_ra_c, pdp_model_v_wd_ra, pdp_model_v_ra_c, pdp_model_v_ra)


ggplot(test_v, aes(velocity, yhat, col = model)) + 
...
```{r}
pdp_model_wd_ra <- pdp_model_wd_ra |>
  mutate(model = "v + wd w/ coordinates")
pdp_model_wd_ra_c <- pdp_model_wd_ra_c |>
  mutate(model = "v + wd w/o coordinates")

pdp_model_wd_samp_ra <- pdp_model_wd_samp_ra |>
  mutate(model = "samp_locations w/ coordinates")
pdp_model_wd_samp_ra_c <- pdp_model_wd_samp_ra_c |>
  mutate(model = "samp_locations w/o coordinates")

pdp_model_wd_sec_ra <- pdp_model_wd_sec_ra |>
  mutate(model = "all predictors w/ coordinates")
pdp_model_wd_sec_ra_c <- pdp_model_wd_sec_ra_c |>
  mutate(model = "all predictors w/o coordinates")

test_wd <- rbind(pdp_model_wd_sec_ra_c, pdp_model_wd_sec_ra, pdp_model_wd_samp_ra_c, pdp_model_wd_samp_ra, pdp_model_wd_ra_c, pdp_model_wd_ra)


ggplot(test_wd, aes(wd_m, yhat, col = model)) + 
  # yhat ist die response variable (k-index, simpson oder shannon)
  geom_line() + 
  labs(
    x = "Water Depth (m)",
