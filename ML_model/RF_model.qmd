...
data_section <- data_section[-seq(c(2), nrow(data_section), by = c(2)), ]
...
#   mtry sample.fraction min.node.size 
#1:    8        0.225604             6                 
rf_model_sec_k_c <- ranger(formula = k_index ~., data = training_sec_k_c, mtry = 8, sample.fraction = 0.225604, min.node.size = 6, importance = "impurity")

# With shannon index as response
#   mtry sample.fraction min.node.size 
#1:    1       0.4528197             1                   
rf_model_sec_s_c <- ranger(formula = shannon ~., data = training_sec_s_c, mtry = 1, sample.fraction = 0.4528197, min.node.size = 1, importance = "impurity")

# With abundance as response
#   mtry sample.fraction min.node.size 
#1:    1       0.7637322             2                   
rf_model_sec_a_c <- ranger(formula = abundance ~., data = training_sec_a_c, mtry = 1, sample.fraction = 0.7637322, min.node.size = 2, importance = "impurity")

# With rel_abundance as response
#   mtry sample.fraction min.node.size 
#1:    1       0.4890119             3                   
rf_model_sec_ra_c <- ranger(formula = rel_abundance ~., data = training_sec_ra_c, mtry = 1, sample.fraction = 0.4890119, min.node.size = 3, importance = "impurity")
...
pred_raster_v_k <- as.data.frame(p_rast_v_k)
rf_v_k <- cbind(pred_v_k, responses_v_k) |>
  dplyr::rename(observed = responses_v_k,
         pred_measured = rf_pred_model_v_k_c,
         pred_rast = pred_raster_v_k)
...
lm_p_value = c(p_v_k, p_v_s, p_v_a, p_v_ra,  
