...
## Deviance explained 
```{r}
# Assuming 'training_v_k_c' is your training data
# and 'k_index' is your response variable

# Step 1: Fit the ranger model
rf_model_v_k_c <- ranger(formula = k_index ~., data = training_v_k_c, mtry = 1, 
                          sample.fraction = 0.2145317, min.node.size = 7, importance = "impurity")

# Step 2: Calculate Total Deviance
observed_values <- training_v_k_c$k_index
predicted_values <- predict(rf_model_v_k_c, data = training_v_k_c)$predictions
total_deviance <- sum((observed_values - mean(observed_values))^2)

# Step 3: Perform Cross-Validation (example using k-fold cross-validation)
set.seed(123)
cv_model <- ranger(formula = k_index ~., data = training_v_k_c, mtry = 1, 
                   sample.fraction = 0.2145317, min.node.size = 7, importance = "impurity",
                   num.trees = 100, num.random.splits = 1, num.fold = 5)

# Extract cross-validation residual deviance
cv_residual_deviance <- cv_model$prediction.error

# Step 4: Calculate Deviance Explained
deviance_explained <- (total_deviance - cv_residual_deviance) / total_deviance

# Print the results
cat("Total Deviance:", total_deviance, "\n")
cat("Cross-Validation Residual Deviance:", cv_residual_deviance, "\n")
