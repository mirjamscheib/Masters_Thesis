...
All predictors: 
- date - ok - evt. komplett separieren
- time - ok - evt. komplett separieren
- ox_sat - NA's sind ein Problem... 
- turbidity - ok 

+ PDP Plots 
Reach_Untersuchungsstelle = as.factor(Reach_Untersuchungsstelle),
         clog_3_class = as.factor(clog_3_class),
         clog_6_class = as.factor(clog_6_class),
...
data_v <- read_delim("Pre_Processing/abiotic_mi_sampling/lab_ml_v.csv") |>
  mutate(velocity = velocity/100) # convert cm/s in to m/s (to have the same units as the hdms)
...
data_section <- read_delim("Pre_Processing/abiotic_mi_sampling/lab_ml_section.csv") |>
  ...
  mutate(date_time = as.POSIXct(date_time, format = "%d.%m.%Y.%H:%M",tz = "UTC"),
         ...
         day = as.integer(format(date_time, "%d")),
         hour = as.integer(format(date_time, "%H")),
         minute = as.integer(format(date_time, "%M")),
         second = as.integer(format(date_time, "%S")),
...
best_rf_v_k <- autotuner_rf_v_k$learners[[1]]
...
```{r}
# 2. Extract the variable importance scores
rf_model <- autotuner_rf_sec_ra$learners[[1]]$train(task_rf_sec_ra)
importance_scores <- mlr3::getFeatureImportance(var_imp_rf_samp_k)
...
### PREDICTORS: SAMPLING LOCATIONS ###
# k_index and sampling locations
differences_samp_k <- rf_predictions_samp_k - responses_samp_k

# shannon index and sampling locations
differences_samp_s <- rf_predictions_samp_s - responses_samp_s

# abundance and sampling locations
differences_samp_a <- rf_predictions_samp_a - responses_samp_a

# relative abundance and sampling locations
differences_samp_ra <- rf_predictions_samp_ra - responses_samp_ra

### PREDICTORS: ALL PREDICTORS ###
# k_index and all predictors
differences_sec_k <- rf_predictions_sec_k - responses_sec_k

# shannon index and all predictors
differences_sec_s <- rf_predictions_sec_s - responses_sec_s

# abundance and all predictors
differences_sec_a <- rf_predictions_sec_a - responses_sec_a

# relative abundance and all predictors
differences_sec_ra <- rf_predictions_sec_ra - responses_sec_ra
```
...
### PREDICTORS: SAMPLING LOCATIONS ###
# k_index and sampling locations
bias_samp_k <- mean(differences_samp_k)

# shannon index and sampling locations
bias_samp_s <- mean(differences_samp_s)

# abundance and sampling locations
bias_samp_a <- mean(differences_samp_a)

# relative abundance and sampling locations
bias_samp_ra <- mean(differences_samp_ra)

### PREDICTORS: ALL PREDICTORS ###
# k_index and all predictors
bias_sec_k <- mean(differences_sec_k)

# shannon index and all predictors
bias_sec_s <- mean(differences_sec_s)

# abundance and all predictors
bias_sec_a <- mean(differences_sec_a)

# relative abundance and all predictors
bias_sec_ra <- mean(differences_sec_ra)
```
...
```{r}
### PREDICTORS: ONLY VELOCITY ###
var_imp_xbrt_v_k <- autotuner_xbrt_v_k$importance()
var_imp_xbrt_v_s <- autotuner_xbrt_v_s$importance()
var_imp_xbrt_v_a <- autotuner_xbrt_v_a$importance()
var_imp_xbrt_v_ra <- autotuner_xbrt_v_ra$importance()

### PREDICTORS: VELOCITY AND WD ###
var_imp_xbrt_wd_k <- autotuner_xbrt_wd_k$importance()
var_imp_xbrt_wd_s <- autotuner_xbrt_wd_s$importance()
var_imp_xbrt_wd_a <- autotuner_xbrt_wd_a$importance()
var_imp_xbrt_wd_ra <- autotuner_xbrt_wd_ra$importance()

### PREDICTORS: SAMPLING LOCATIONS ###
var_imp_xbrt_samp_k <- autotuner_xbrt_samp_k$importance()
var_imp_xbrt_samp_s <- autotuner_xbrt_samp_s$importance()
var_imp_xbrt_samp_a <- autotuner_xbrt_samp_a$importance()
var_imp_xbrt_samp_ra <- autotuner_xbrt_samp_ra$importance()


### PREDICTORS: ALL PREDICTORS ###
var_imp_xbrt_sec_k <- autotuner_xbrt_sec_k$importance()
var_imp_xbrt_sec_s <- autotuner_xbrt_sec_s$importance()
var_imp_xbrt_sec_a <- autotuner_xbrt_sec_a$importance()
var_imp_xbrt_sec_ra <- autotuner_xbrt_sec_ra$importance()

# bind results together
vi_xbrt_v <- rbind(var_imp_xbrt_v_k, var_imp_xbrt_v_s, var_imp_xbrt_v_a, var_imp_xbrt_v_ra)
vi_xbrt_wd <- rbind(var_imp_xbrt_wd_k, var_imp_xbrt_wd_s, var_imp_xbrt_wd_a, var_imp_xbrt_wd_ra)
vi_xbrt_samp <- rbind(var_imp_xbrt_samp_k, var_imp_xbrt_samp_s, var_imp_xbrt_samp_a, var_imp_xbrt_samp_ra)
vi_xbrt_sec <- rbind(var_imp_xbrt_sec_k, var_imp_xbrt_sec_s, var_imp_xbrt_sec_a, var_imp_xbrt_sec_ra)

# write results into .csv
write.csv(vi_xgbrt_v, "ML_model/vi_xgbrt_v.csv", row.names = FALSE)
write.csv(vi_xgbrt_wd, "ML_model/vi_xgbrt_wd.csv", row.names = FALSE)
write.csv(vi_xgbrt_samp, "ML_model/vi_xgbrt_samp.csv", row.names = FALSE)
write.csv(vi_xgbrt_sec, "ML_model/vi_xgbrt_sec.csv", row.names = FALSE)
```
...
# relative abundance and velocity 
xbrt_predictions_v_ra <- predict(autotuner_xbrt_v_ra, newdata = test_v_ra)
...
# relative abundance and velocity + wd
xbrt_predictions_wd_ra <- predict(autotuner_xbrt_wd_ra, newdata = test_wd_ra)

### PREDICTORS: SAMPLING LOCATIONS ###
# k_index and sampling locations
xbrt_predictions_samp_k <- predict(autotuner_xbrt_samp_k, newdata = test_samp_k)

# shannon index and sampling locations
xbrt_predictions_samp_s <- predict(autotuner_xbrt_samp_s, newdata = test_samp_s)

# abundance and sampling locations
xbrt_predictions_samp_a <- predict(autotuner_xbrt_samp_a, newdata = test_samp_a)

# relative abundance and sampling locations
xbrt_predictions_samp_ra <- predict(autotuner_xbrt_samp_ra, newdata = test_samp_ra)

### PREDICTORS: ALL PREDICTORS ###
# k_index and all predictors
xbrt_predictions_sec_k <- predict(autotuner_xbrt_sec_k, newdata = test_sec_k)

# shannon index and all predictors
xbrt_predictions_sec_s <- predict(autotuner_xbrt_sec_s, newdata = test_sec_s)

# abundance and all predictors
xbrt_predictions_sec_a <- predict(autotuner_xbrt_sec_a, newdata = test_sec_a)

# relative abundance and all predictors
xbrt_predictions_sec_ra <- predict(autotuner_xbrt_sec_ra, newdata = test_sec_ra)
```
...
# relative abundance and velocity 
brt_r2_v_ra <- R2(pred = xbrt_predictions_v_ra, obs = responses_v_ra)
...
# relative abundance and velocity + wd 
brt_r2_wd_ra <- R2(pred = xbrt_predictions_wd_ra, obs = responses_wd_ra)

### PREDICTORS: SAMPLING LOCATIONS ###
# k_index and sampling locations
brt_r2_samp_k <- R2(pred = xbrt_predictions_samp_k, obs = responses_samp_k)

# shannon index and sampling locations 
brt_r2_samp_s <- R2(pred = xbrt_predictions_samp_s, obs = responses_samp_s)

# abundance and sampling locations 
brt_r2_samp_a <- R2(pred = xbrt_predictions_samp_a, obs = responses_samp_a)

# relative abundance and sampling locations
brt_r2_samp_ra <- R2(pred = xbrt_predictions_samp_ra, obs = responses_samp_ra)

### PREDICTORS: ALL PREDICTORS ###
# k_index and all predictors
brt_r2_sec_k <- R2(pred = xbrt_predictions_sec_k, obs = responses_sec_k)

# shannon index and all predictors 
brt_r2_sec_s <- R2(pred = xbrt_predictions_sec_s, obs = responses_sec_s)

# abundance and all predictors
brt_r2_sec_a <- R2(pred = xbrt_predictions_sec_a, obs = responses_sec_a)

# relative abundance and all predictors 
brt_r2_sec_ra <- R2(pred = xbrt_predictions_sec_ra, obs = responses_sec_ra)
```
...
# relative abundance and velocity
brt_rmse_v_ra <- rmse(actual = responses_v_ra, predicted = xbrt_predictions_v_ra)
brt_mae_v_ra <- mae(actual = responses_v_ra, predicted = xbrt_predictions_v_ra)
...
# relative abundance and velocity + wd
brt_rmse_wd_ra <- rmse(actual = responses_wd_ra, predicted = xbrt_predictions_wd_ra)
brt_mae_wd_ra <- mae(actual = responses_wd_ra, predicted = xbrt_predictions_wd_ra)

### PREDICTORS: SAMPLING LOCATIONS ###
# k_index and sampling locations
brt_rmse_samp_k <- rmse(actual = responses_samp_k, predicted = xbrt_predictions_samp_k)
brt_mae_samp_k <- mae(actual = responses_samp_k, predicted = xbrt_predictions_samp_k)

# shannon index and sampling locations
brt_rmse_samp_s <- rmse(actual = responses_samp_s, predicted = xbrt_predictions_samp_s)
brt_mae_samp_s <- mae(actual = responses_samp_s, predicted = xbrt_predictions_samp_s)

# abundance and sampling locations
brt_rmse_samp_a <- rmse(actual = responses_samp_a, predicted = xbrt_predictions_samp_a)
brt_mae_samp_a <- mae(actual = responses_samp_a, predicted = xbrt_predictions_samp_a)

# relative abundance and sampling locations
brt_rmse_samp_ra <- rmse(actual = responses_samp_ra, predicted = xbrt_predictions_samp_ra)
brt_mae_samp_ra <- mae(actual = responses_samp_ra, predicted = xbrt_predictions_samp_ra)

### PREDICTORS: ALL PREDICTORS ###
# k_index and all predictors
brt_rmse_sec_k <- rmse(actual = responses_sec_k, predicted = xbrt_predictions_sec_k)
brt_mae_sec_k <- mae(actual = responses_sec_k, predicted = xbrt_predictions_sec_k)

# shannon index and all predictors
brt_rmse_sec_s <- rmse(actual = responses_sec_s, predicted = xbrt_predictions_sec_s)
brt_mae_sec_s <- mae(actual = responses_sec_s, predicted = xbrt_predictions_sec_s)

# abundance and all predictors
brt_rmse_sec_a <- rmse(actual = responses_sec_a, predicted = xbrt_predictions_sec_a)
brt_mae_sec_a <- mae(actual = responses_sec_a, predicted = xbrt_predictions_sec_a)

# relative abundance and all predictors
brt_rmse_sec_ra <- rmse(actual = responses_sec_ra, predicted = xbrt_predictions_sec_ra)
brt_mae_sec_ra <- mae(actual = responses_sec_ra, predicted = xbrt_predictions_sec_ra)
```
...
# relative abundance and velocity
brt_differences_v_ra <- xbrt_predictions_v_ra - responses_v_ra
...
# relative abundance and velocity + wd
brt_differences_wd_ra <- xbrt_predictions_wd_ra - responses_wd_ra

### PREDICTORS: SAMPLING LOCATIONS ###
# k_index and sampling locations
brt_differences_samp_k <- xbrt_predictions_samp_k - responses_samp_k

# shannon index and sampling locations
brt_differences_samp_s <- xbrt_predictions_samp_s - responses_samp_s

# abundance and sampling locations
brt_differences_samp_a <- xbrt_predictions_samp_a - responses_samp_a

# relative abundance and sampling locations
brt_differences_samp_ra <- xbrt_predictions_samp_ra - responses_samp_ra

### PREDICTORS: ALL PREDICTORS ###
# k_index and all predictors
brt_differences_sec_k <- xbrt_predictions_sec_k - responses_sec_k

# shannon index and all predictors
brt_differences_sec_s <- xbrt_predictions_sec_s - responses_sec_s

# abundance and all predictors
brt_differences_sec_a <- xbrt_predictions_sec_a - responses_sec_a

# relative abundance and all predictors
brt_differences_sec_ra <- xbrt_predictions_sec_ra - responses_sec_ra
```
...
# relative abundance and velocity
brt_bias_v_ra <- mean(brt_differences_v_ra)
...
# relative abundance and velocity + wd
brt_bias_wd_ra <- mean(brt_differences_wd_ra)

### PREDICTORS: SAMPLING LOCATIONS ###
# k_index and sampling locations
brt_bias_samp_k <- mean(brt_differences_samp_k)

# shannon index and sampling locations
brt_bias_samp_s <- mean(brt_differences_samp_s)

# abundance and sampling locations
brt_bias_samp_a <- mean(brt_differences_samp_a)

# relative abundance and sampling locations
brt_bias_samp_ra <- mean(brt_differences_samp_ra)

### PREDICTORS: ALL PREDICTORS ###
# k_index and all predictors
brt_bias_sec_k <- mean(brt_differences_sec_k)

# shannon index and all predictors
brt_bias_sec_s <- mean(brt_differences_sec_s)

# abundance and all predictors
brt_bias_sec_a <- mean(brt_differences_sec_a)

# relative abundance and all predictors
brt_bias_sec_ra <- mean(brt_differences_sec_ra)
```
...
# relative abundance and velocity
brt_nse_v_ra <- 1 - sum((responses_v_ra - xbrt_predictions_v_ra)^2) / sum((responses_v_ra - mean(responses_v_ra))^2)
...
# relative abundance and velocity + wd
brt_nse_wd_ra <- 1 - sum((responses_wd_ra - xbrt_predictions_wd_ra)^2) / sum((responses_wd_ra - mean(responses_wd_ra))^2)

### PREDICTORS: SAMPLING LOCATIONS ###
# k_index and sampling locations
brt_nse_samp_k <- 1 - sum((responses_samp_k - xbrt_predictions_samp_k)^2) / sum((responses_samp_k - mean(responses_samp_k))^2)

# shannon index and sampling locations
brt_nse_samp_s <- 1 - sum((responses_samp_s - xbrt_predictions_samp_s)^2) / sum((responses_samp_s - mean(responses_samp_s))^2)

# abundance and sampling locations
brt_nse_samp_a <- 1 - sum((responses_samp_a - xbrt_predictions_samp_a)^2) / sum((responses_samp_a - mean(responses_samp_a))^2)

# relative abundance and sampling locations
brt_nse_samp_ra <- 1 - sum((responses_samp_ra - xbrt_predictions_samp_ra)^2) / sum((responses_samp_ra - mean(responses_samp_ra))^2)

### PREDICTORS: ALL PREDICTORS ###
# k_index and all predictors
brt_nse_sec_k <- 1 - sum((responses_sec_k - xbrt_predictions_sec_k)^2) / sum((responses_sec_k - mean(responses_sec_k))^2)

# shannon index and all predictors
brt_nse_sec_s <- 1 - sum((responses_sec_s - xbrt_predictions_sec_s)^2) / sum((responses_sec_s - mean(responses_sec_s))^2)

# abundance and all predictors
brt_nse_sec_a <- 1 - sum((responses_sec_a - xbrt_predictions_sec_a)^2) / sum((responses_sec_a - mean(responses_sec_a))^2)

# relative abundance and all predictors
brt_nse_sec_ra <- 1 - sum((responses_sec_ra - xbrt_predictions_sec_ra)^2) / sum((responses_sec_ra - mean(responses_sec_ra))^2)
```

## Write Evaluation Results 
```{r}
# write evaluation metrics into df
metric_results <- data.frame(r2 = c(r2_v_k, r2_v_s, r2_v_a, r2_v_ra, 
                             r2_wd_k, r2_wd_s, r2_wd_a, r2_wd_ra, 
                             r2_samp_k, r2_samp_s, r2_samp_a, r2_samp_ra, 
                             r2_sec_k, r2_sec_s, r2_sec_a, r2_sec_ra,
                             brt_r2_v_k, brt_r2_v_s, brt_r2_v_a, brt_r2_v_ra,
                             brt_r2_wd_k, brt_r2_wd_s, brt_r2_wd_a, brt_r2_wd_ra,
                             brt_r2_samp_k, brt_r2_samp_s, brt_r2_samp_a, brt_r2_samp_ra,
                             brt_r2_sec_k, brt_r2_sec_s, brt_r2_sec_a, brt_r2_sec_ra),
                      nse = c(nse_v_k, nse_v_s, nse_v_a, nse_v_ra,
                              nse_wd_k, nse_wd_s, nse_wd_a, nse_wd_ra,
                              nse_samp_k, nse_samp_s, nse_samp_a, nse_samp_ra,
                              nse_sec_k, nse_sec_s, nse_sec_a, nse_sec_ra,
                              brt_nse_v_k, brt_nse_v_s, brt_nse_v_a, brt_nse_v_ra, 
                              brt_nse_wd_k, brt_nse_wd_s, brt_nse_wd_a, brt_nse_wd_ra,
                              brt_nse_samp_k, brt_nse_samp_s, brt_nse_samp_a, brt_nse_samp_ra, 
                              brt_nse_sec_k, brt_nse_sec_s, brt_nse_sec_a, brt_nse_sec_ra),
                        bias = c(bias_v_k, bias_v_s, bias_v_a, bias_v_ra,
                                 bias_wd_k, bias_wd_s, bias_wd_a, bias_wd_ra,
                                 bias_samp_k, bias_samp_s, bias_samp_a, bias_samp_ra,
                                 bias_sec_k, bias_sec_s, bias_sec_a, bias_sec_ra,
                                 brt_bias_v_k, brt_bias_v_s, brt_bias_v_a, brt_bias_v_ra, 
                                 brt_bias_wd_k, brt_bias_wd_s, brt_bias_wd_a, brt_bias_wd_ra,
                                 brt_bias_samp_k, brt_bias_samp_s, brt_bias_samp_a,
                                 brt_bias_samp_ra, 
                                 brt_bias_sec_k, brt_bias_sec_s, brt_bias_sec_a, brt_bias_sec_ra),
                        rmse = c(rmse_v_k, rmse_v_s, rmse_v_a, rmse_v_ra,
                                 rmse_wd_k, rmse_wd_s, rmse_wd_a, rmse_wd_ra,
                                 rmse_samp_k, rmse_samp_s, rmse_samp_a, rmse_samp_ra,
                                 rmse_sec_k, rmse_sec_s, rmse_sec_a, rmse_sec_ra,
                                 brt_rmse_v_k, brt_rmse_v_s, brt_rmse_v_a, brt_rmse_v_ra,
                                 brt_rmse_wd_k, brt_rmse_wd_s, brt_rmse_wd_a, brt_rmse_wd_ra,
                                 brt_rmse_samp_k, brt_rmse_samp_s, brt_rmse_samp_a,
                                 brt_rmse_samp_ra,
                                 brt_rmse_sec_k, brt_rmse_sec_s, brt_rmse_sec_a, brt_rmse_sec_ra), 
                         mae = c(mae_v_k, mae_v_s, mae_v_a, mae_v_ra,
                                 mae_wd_k, mae_wd_s, mae_wd_a, mae_wd_ra,
                                 mae_samp_k, mae_samp_s, mae_samp_a, mae_samp_ra,
                                 mae_sec_k, mae_sec_s, mae_sec_a, mae_sec_ra,
                                 brt_mae_v_k, brt_mae_v_s, brt_mae_v_a, brt_mae_v_ra,
                                 brt_mae_wd_k, brt_mae_wd_s, brt_mae_wd_a, brt_mae_wd_ra,
                                 brt_mae_samp_k, brt_mae_samp_s, brt_mae_samp_a, brt_mae_samp_ra,
                                 brt_mae_sec_k, brt_mae_sec_s, brt_mae_sec_a, brt_mae_sec_ra),
                         predictor = c("velocity", "velocity", "velocity", "velocity",
                                       ...
                                       "all_predictors", "all_predictors", "all_predictors",
                                       "all_predictors"),
                         index = c("k-index", "shannon index", "abundance", "rel_abundance",
                                   "k-index", "shannon index", "abundance", "rel_abundance",
                                   "k-index", "shannon index", "abundance", "rel_abundance",
                                   "k-index", "shannon index", "abundance", "rel_abundance",
                                   "k-index", "shannon index", "abundance", "rel_abundance",
                                   "k-index", "shannon index", "abundance", "rel_abundance",
                                   "k-index", "shannon index", "abundance", "rel_abundance",
                                   "k-index", "shannon index", "abundance", "rel_abundance"),
                      model = c("rf", "rf", "rf", "rf", "rf", "rf", "rf", "rf", "rf", "rf", "rf",
                                "rf", "rf", "rf", "rf", "rf", "xgbrt", "xgbrt", "xgbrt", "xgbrt",
                                "xgbrt", "xgbrt", "xgbrt", "xgbrt", "xgbrt", "xgbrt", "xgbrt",
                                "xgbrt", "xgbrt", "xgbrt", "xgbrt", "xgbrt"))

# write results into a .csv 
write.csv(metric_results, "Stat_Analysis/ml_evaluation_metrics.csv", row.names = FALSE)
```
