...
einfachste Variante die möglich ist test und train dataset zu trennen (hydrodyn. modelle)
...
data_v_sp <-  st_as_sf(training_v, coords =  c("x", "y"))
st_crs(data_v_sp) <- "EPSG: 2056"

data_wd_sp <-  st_as_sf(training_wd, coords =  c("x", "y"))
...
rf_model_v <- ranger(formula = k_index ~., data = test_v, mtry = 1, sample.fraction = 0.2059293, min.node.size = 10)

# Create and train the random forest model with velocity + wd
#      mtry sample.fraction min.node.size learner_param_vals  x_domain regr.rmse
# 1:    1       0.2059293            10          <list[4]> <list[3]>  0.141822
rf_model_wd <- ranger(formula = k_index ~., data = test_wd, mtry = 1, sample.fraction = 0.2059293, min.node.size = 10)
