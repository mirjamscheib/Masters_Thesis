...
lab_ml_model_v <- lab_new |>
  ...
  dplyr::select(Reach_Untersuchungsstelle, `Flow_velocity_v60_cm/s`, x, y, shannon, abundance, k_index, rel_abundance) |>
...
lab_ml_model_wd_v <- lab_new |>
  mutate(k_index = 0.4*(Number_Taxa/max(Number_Taxa, na.rm = TRUE))
         ...
         + 0.2*(EPT_Taxa/max(EPT_Taxa, na.rm = TRUE))
         + 0.1*(abundance/max(abundance, na.rm = TRUE))) |>
  dplyr::select(Reach_Untersuchungsstelle, `Flow_velocity_v60_cm/s`, Water_depth_cm, x, y, shannon, abundance, k_index, rel_abundance) |>
  rename(velocity = `Flow_velocity_v60_cm/s`,
         wd_m = Water_depth_cm) |>
  mutate(x = as.numeric(x),
         y = as.numeric(y),
...
```
  ...
  dplyr::select(Date_of_survey.x, Time_of_survey, Reach_Untersuchungsstelle, `Flow_velocity_v60_cm/s`, Water_depth_cm, x, y, z, `DIstance_Wasseranschlaglinie_m`, `Category_Flow_velocity_5_Bewohnbarkeits-klasse`, Clogging_3_Klassen, Clogging_6_Klassen, Substrate_type, `Algae_cover_Bewuchsdichte_5_Kategorien_1-5`, `Moos_cover_Deckungsgrad_6_Kategorien_0-5`, `CPOM_cover_Deckungsgrad_5_Kategorien_1-5`, shannon, abundance, k_index, rel_abundance) |>
```{r}
...
lab_ml_model_sampling <- lab_new |>
  mutate(k_index = 0.4*(Number_Taxa/max(Number_Taxa, na.rm = TRUE))
         ...
         + 0.2*(EPT_Taxa/max(EPT_Taxa, na.rm = TRUE))
         + 0.1*(abundance/max(abundance, na.rm = TRUE))) |>
  dplyr::select(Reach_Untersuchungsstelle, `Flow_velocity_v60_cm/s`, Water_depth_cm, x, y, z, `DIstance_Wasseranschlaglinie_m`, `Category_Flow_velocity_5_Bewohnbarkeits-klasse`, Clogging_3_Klassen, Clogging_6_Klassen, Substrate_type, `Algae_cover_Bewuchsdichte_5_Kategorien_1-5`, `Moos_cover_Deckungsgrad_6_Kategorien_0-5`, `CPOM_cover_Deckungsgrad_5_Kategorien_1-5`, shannon, abundance, k_index, rel_abundance) |>
  rename(velocity = `Flow_velocity_v60_cm/s`,
         wd_m = Water_depth_cm,
...
lab_ox_sat <- lab_new[ ,c("Oxygen_saturation_%_before", "Oxygen_saturation_%_during", "Oxygen_saturation_%_after")]
...
lab_ml_model_section <- lab_new |>
  mutate(k_index = 0.4*(Number_Taxa/max(Number_Taxa, na.rm = TRUE))
         ...
         + 0.2*(EPT_Taxa/max(EPT_Taxa, na.rm = TRUE))
         + 0.1*(abundance/max(abundance, na.rm = TRUE))) |>
  dplyr::select(-Transect, -Sampling_point, -Abiotic_measured_by.x, -Abiotic_measured_by.y, -River.x, -`RTK-GPS_Nr`, -`MZB_kick-net_taken_by`, -Varia, -Varia_before, -Varia_during, -Varia_after, -Gauging_station, -Sampling_site, -join_key) |>
  rename(date = Date_of_survey.x,
         time = Time_of_survey,
         ...
         ox_sat_before = `Oxygen_saturation_%_before`,
         ox_sat_during = `Oxygen_saturation_%_during`,
         ox_sat_after = `Oxygen_saturation_%_after`,
...
# write results into new .csv file 
write_xlsx(lab_ml_model_sampling,"Pre_Processing\\abiotic_mi_sampling\\lab_ml_sampling.xlsx") 
write_xlsx(lab_ml_model_section,"Pre_Processing\\abiotic_mi_sampling\\lab_ml_section.xlsx")
```
...
lab_univariate <- lab_ml_model_wd_v |>
  dplyr::select(Reach_Untersuchungsstelle, velocity, wd_m, x, y, shannon, abundance, k_index) |>
  mutate(x = as.numeric(x),
         y = as.numeric(y),
         velocity = velocity/100,
         wd_m = wd_m/100)
