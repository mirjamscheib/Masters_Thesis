...
clog_6_int <- c("none" = 0, "none " = 0, "none-light" = 1, "none-slight" = 2, "slight" = 3, "slight-medium" = 4, "medium-strong" = 5, "strong" = 6)
...
data_new <- dplyr::left_join(data, aggr_gis, by = "reach")
...
# save as .csv file
write.csv(data_int, "Pre_Processing/abiotic_mi_sampling/ml_selected_var.csv", row.names = FALSE)  

write.csv(data_int_2, "Pre_Processing/abiotic_mi_sampling/ml_sel_var_2.csv", row.names = FALSE)  
```

```{r}
# Check for duplicates in the 'reach' column of 'data'
duplicated_reach <- data$duplicated("reach")

data <- aggregate(. ~ reach, data, mean)
```
