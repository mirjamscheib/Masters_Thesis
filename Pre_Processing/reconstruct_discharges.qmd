...
```{r}
# load data 
data <- read_delim("Pre_Processing/abiotic_mi_sampling/Abiotic_MI_Field.csv") |>
...
```{r}
library(Metrics)
regr_analysis <- na.omit(regr_analysis)
rmse(actual = regr_analysis$velocity, predicted = regr_analysis$v_ezg)
rmse(actual = regr_analysis$wd_cm, predicted = regr_analysis$wd_cm_ezg)

rmse(actual = regr_analysis$velocity, predicted = regr_analysis$v_q)
rmse(actual = regr_analysis$wd_cm, predicted = regr_analysis$wd_cm_q)

lm_v_ezg <- lm(velocity ~ v_ezg, data = regr_analysis)
summary(lm_v_ezg)

lm_wd_ezg <- lm(wd_cm ~ wd_cm_ezg, data = regr_analysis)
summary(lm_wd_ezg)

ggplot(regr_analysis, aes(wd_cm, wd_cm_ezg)) +
  geom_point() +
  geom_smooth(method = "lm")

ggplot(regr_analysis, aes(velocity, v_ezg)) +
  geom_point() +
  geom_smooth(method = "lm")
```
