...
data <- read_delim("Pre_Processing/abiotic_mi_sampling/Abiotic_MI_Field.csv") |>
  mutate(reach = as.factor(reach), # convert character variables into categorical variables (factors with levels) for ML algorithm 
         velocity = velocity/100,  # convert cm/s in to m/s (to have the same units as the hdms)
         wd_m = wd_cm/100) # convert cm in to m (to have the same units as the hdms)
