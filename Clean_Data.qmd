...
# Remove single letters from the specified row - i want to remove NA from the headers
...
# geht im moment noch nicht, weil ich noch nicht herausgefunden habe, wie ich die 1. Zeile von links nach rechts so auffülle wie unter # Lab Data wo ich eine column von oben nach unten auffülle. Die 1. Zeile soll dann mit der 2. Zeile verbunden werden damit wird nur noch eine Zeile mit Header Info haben. Also z.B. Allogamus_count, Allogamus_projection, Allogamus_total (weil im Excel Allogamus als Einzelüberschrift für count, projection und total gilt wird es im R nur einmal eingefüllt.)

# Im Notfall kann ich das auch im Excel machen, geht aber ziemlich lange
```
...
t_lab <- t_lab |>
  fill(`  Sampling_site`) |>
  select(-...1) |>
  function(x) gsub(`  Transekt - Kick`, "T", "", x) # wieso geht das nicht, dass ich in der Spalte Transekt-kick das T aus dem character jeweils entfernen kann? 
# Nur wenn das T aus der Zahlenkombination draussen ist, kann ich den Join_key erstellen aus "river-transect-sampling_point"
```
...
# Join Dataframes 
Evt. hier dann noch columns R tauglich anschreiben 
```{r}
