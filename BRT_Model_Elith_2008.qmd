...
```{r}
# example data
model.data <- read.csv("BRT_Model/model.data.csv")

# make factor 
model.data$Method <- as.factor(model.data$Method)

# Load Data
data <- read.csv("abiotic_mi_sampling/Lab_k_index.csv")

# choose only relevant variables 
# data <- data[ ,c(7, 9,27)] # 22:24 = x, y, z
data <- data[ ,c(2, 6, 7, 9, 14:16, 20, 22:24, 27)]

data <- data |>
  rename(wd_cm = Water_depth_cm, 
         v_cm_s = `Flow_velocity_v60_cm.s`,
         reach = Reach_Untersuchungsstelle,
...
data_x_l2 <- gbm.step(data= data, 
    gbm.x = c(1,3,4), # reach, wd, v
    gbm.y = 12,
    family = "laplace",
    tree.complexity = 2,
    learning.rate = 0.002,
...
data_y_l2 <- gbm.step(data = data, 
    gbm.x = c(3,4), # reach, wd, v
    gbm.y = 12,
    family = "laplace",
    tree.complexity = 2,
...
angaus.tc5.lr005 <- gbm.step(data=model.data, 
    gbm.x = 3:14, # auch hier wäre 3:14 aber hat irgendwie probleme 
    gbm.y = 2,
    family = "bernoulli",
    tree.complexity = 5,
    learning.rate = 0.005,
