...
```{r}
# Load Data
data <- read_delim("Data/Data_Complete/Lab_k_index.csv")

data <- data[ ,c(7, 9, 27)]
# data <- data[ ,-c(1,2,3,4,5, 6, 10, 13, 17, 18:21, 25, 26)]
...
# Units of raster files: 
# velocity is in m/s
# depth is in m

# change variable type which we want to predict 
# data$Reach_Untersuchungsstelle <- as.factor(data$Reach_Untersuchungsstelle)
# data$Clogging_6_Klassen <- as.factor(data$Clogging_6_Klassen)
# data$Clogging_3_Klassen <- as.factor(data$Clogging_3_Klassen)

data$k_index <- as.factor(data$k_index)
```
...
env_data <- stack("Data/Rasters/env_rasters.grd")

stack("Data/Rasters/env_rasters.grd")
...
# predict only single values from the whole model 
```{r}
# read raster 
landquart <- raster("Data/raster_files/Landquart_results_velocity_9.tif")

plot(landquart)

# select value to be predicted 
input_data <- test_data[["Landquart_results_velocity_9"]]

# predict values 
predictions <- raster::predict(model = boost_model, object = landquart)
```
